 ...
 * 模块加载器实现
 */
public class RpcModuleLoaderImpl implements RpcModuleLoader, ApplicationContextAware {

    private static final Logger LOGGER = LoggerFactory.getLogger(RpcModuleLoaderImpl.class);
    ...
    @Override
    public RpcModule load(ModuleConfig moduleConfig) {
        ...
        return new RpcSpringModule(moduleClassLoader, moduleConfig, moduleConfig.getVersion(), moduleConfig.getName(), moduleApplicationContext);
    }

    @Override
    public void unload(RpcModule module) {
        if (module != null) {
            module.destroy();
        }
    }
    ...
    private ConfigurableApplicationContext loadModuleApplication(ModuleConfig moduleConfig, List<String>
        ...
        //获取模块的ClassLoader
        moduleClassLoader = new ModuleClassLoader(moduleConfig.getModuleUrl(), applicationContext
    ...
    private Properties getProperties(ModuleConfig moduleConfig) {
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("RpcModule Properties: {}", properties);
        }
        return properties;
    }
